...
export default function SelectedWorks() {
  ...
  useGSAP(() => {
    cardsRef.current.forEach((cardWrapper) => {
      ...
      const cardInner = cardWrapper.querySelector('.project-card-inner');
  ...
  return (
    ...
    <section ref={containerRef} className="relative w-full bg-[#Ffffff] px-10 md:px-12 pt-10 pb-24">
      ...
      {/* Card Stacking Area */}
      <div className="relative w-full max-w-[1440px] mx-auto pb-[20vh]">
        {projects.map((project, index) => (
          <div
            key={project.id}
            ref={addToRefs}
            data-cursor="case-study"
            className="sticky top-24 w-full cursor-none"
            style={{ 
              zIndex: index + 1,
          ...
          >
            ...
            <div 
              className="project-card-inner absolute top-0 left-0 w-full overflow-hidden rounded-[0px] shadow-[0_0_40px_rgba(0,0,0,0.15)] will-change-[height]"
              style={{ backgroundColor: project.bgColor }}
            >
              {/* Inner Fixed Container with Flex Column Layout */}
              <div className="absolute top-0 left-0 w-full h-[92vh] flex flex-col p-8 md:p-12">
                ...
                <div className="relative w-full flex-grow flex items-center">
                  <div className="w-full h-px bg-white/20 relative">
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 bg-white rounded-l-2x4 rounded-r-2x4  py-3 px-6 shadow-2xl flex items-center gap-3">
                ...
                <div className="relative flex flex-col md:flex-row justify-between items-end w-full gap-10">
                  ...
                  <div className="flex items-end gap-4 w-full md:w-auto h-[30vh]">
                    {project.mockups.map((src, i) => (
                      <div 
                        key={i} 
                        className={`w-1/3 md:w-56 bg-black/10 rounded-2xl overflow-hidden backdrop-blur-sm relative shadow-xl
                          ${i === 0 ? 'h-full' : i === 1 ? 'h-full' : 'h-full'}`}
                      >
                        {/* Fallback styling for when images load */}
                        <img 
                          src={src} 
                          alt={`Mockup ${i + 1}`} 
                          className="w-full h-full object-cover object-top"
                          // In case image path fails, it falls back nicely to the glass effect
                          onError={(e) => {
